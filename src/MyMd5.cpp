...
char *MD5_file(char *path, int md5_len,bool type)
{
	HINTERNET hSession;
	FILE *fp = NULL;
	MD5_CTX mdContext;
	...
	if (type == 0){
		...
		{
			HINTERNET hRequest;
			hRequest = InternetOpenUrlA(hSession, path, NULL, 0, INTERNET_FLAG_RELOAD, 0);
			if (hRequest != NULL)
			{
				DWORD dwBytesRead;
				...
				if (InternetReadFile(hRequest, szBuffer, BUF_SIZE*2, &dwBytesRead))
				{
					...
					bytes = 1024;
					memcpy(data, temp_data + BUF_SIZE, BUF_SIZE);
				}
			}
			else
				return NULL;
		...
		if ((bytes = fread(data, 1, BUF_SIZE, fp)) != 0)
		{
			return NULL;
		}
	}
